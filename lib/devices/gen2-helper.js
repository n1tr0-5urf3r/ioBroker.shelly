...
function addBluTrvDevice(deviceObj, sensorId) {
    ...
    // TODO put this into shellybluwg3.js and then add a bluTRV Device for each detected ID
    // Somehow, the returned list is always empty / null
    deviceObj['System.GetComponents'] = {
        mqtt: {
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => JSON.stringify({
                id: self.getNextMsgId(),
            ...
            mqtt_publish: '<mqttprefix>/rpc',
            mqtt_publish_funct: value => {
                let trv_ids = [];
                const valueObj = JSON.parse(value);
    
                // Log the components to debug
                console.log("Components:", valueObj.components);
    
                for (const c of valueObj.components) {
                    console.log("Checking component:", c); // Log each component
                    if (c.key?.startsWith('blutrv:') && typeof c.config?.id === 'number') {
                        trv_ids.push(c.config.id);
                    }
                }
    
                console.log("TRV IDs:", trv_ids); // Log the result
                return JSON.stringify(trv_ids);
            },
        },
        common: {
            name: { en: 'List Components with Config' },
            type: 'mixed',
            role: 'json',
            read: true,
            write: false,
