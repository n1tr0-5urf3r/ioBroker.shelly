...
/**
 * Adds support for a Shelly BLU TRV device via a ShellyBluGwG3
 *
 * @param {object} deviceObj - The object to populate with state definitions
 */
function addBluTrvDevice(deviceObj) {

    const trv_list = this.getStateAsync("shelly.0.shellyblugwg3#34cdb0789768#1.PairedDevices.BluTRV");
    this.log.warn(JSON.stringify(trv_list));

    for (const trv of JSON.parse(deviceObj["PairedDevices.BluTRV"])) {
        const sensorId = trv.id;

        deviceObj[`BluTRV${sensorId}.TargetTemperature`] = {
            mqtt: {
                http_publish: `/rpc/BluTrv.GetStatus?id=${sensorId}`,
                http_publish_funct: (value) => value ? JSON.parse(value).target_C : undefined,
                mqtt_cmd: '<mqttprefix>/rpc',
                mqtt_cmd_funct: (value, self) => JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'BluTrv.Call',
                    params: {
                        ...
                        params: {
                            id: 0, // always 0 for single TRV devices
                            target_C: value
                        }
                    }
                }),            
        ...
        deviceObj[`BluTRV${sensorId}.CurrentTemperature`] = {
            mqtt: {
                http_publish: `/rpc/BluTrv.GetStatus?id=${sensorId}`,
                http_publish_funct: (value) => value ? JSON.parse(value).current_C : undefined,
            },
            common: {
                name: { en: 'Current Temperature' },
                type: 'number',
                role: 'value.temperature',
                unit: '°C',
                read: true,
        ...
        deviceObj[`BluTRV${sensorId}.ValvePosition`] = {
            mqtt: {
                http_publish: `/rpc/BluTrv.GetStatus?id=${sensorId}`,
                http_publish_funct: (value) => value ? JSON.parse(value).pos : undefined,
            },
            common: {
                name: { en: 'Valve Position' },
                type: 'number',
                role: 'value',
                unit: '%',
                read: true,
        ...
        deviceObj[`BluTRV${sensorId}.Battery`] = {
            mqtt: {
                http_publish: `/rpc/BluTrv.GetStatus?id=${sensorId}`,
                http_publish_funct: (value) => value ? JSON.parse(value).battery : undefined,
            },
            common: {
                name: { en: 'Battery Level' },
                type: 'number',
                role: 'value.battery',
                unit: '%',
                read: true,
        ...
        deviceObj[`BluTRV${sensorId}.Errors`] = {
            mqtt: {
                http_publish: `/rpc/BluTrv.GetStatus?id=${sensorId}`,
                http_publish_funct: (value) => value ? JSON.stringify(JSON.parse(value).errors) : undefined,
            },
            common: {
                name: { en: 'Error States' },
                type: 'string',
                role: 'json',
                read: true,
                write: false,
        ...
        deviceObj[`BluTRV${sensorId}.CheckForUpdates`] = {
            mqtt: {
                mqtt_cmd: '<mqttprefix>/rpc',
                mqtt_cmd_funct: (value, self) => JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'BluTrv.CheckForUpdates',
                    params: { id: sensorId },
                }),
            },
            common: {
                name: { en: 'Check for Firmware Updates' },
                type: 'boolean',
                role: 'button',
                read: false,
                write: true,
        ...
        deviceObj[`BluTRV${sensorId}.UpdateFirmware`] = {
            mqtt: {
                mqtt_cmd: '<mqttprefix>/rpc',
                mqtt_cmd_funct: (value, self) => JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'BluTrv.UpdateFirmware',
                    params: { id: sensorId },
                }),
            },
            common: {
                name: { en: 'Trigger Firmware Update' },
                type: 'boolean',
                role: 'button',
                read: false,
                write: true,
...
function detectPairedBluTRVs(deviceObj){
    deviceObj['PairedDevices.BluTRV'] = {
        mqtt: {
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => JSON.stringify({
                id: self.getNextMsgId(),
            ...
            http_publish: '/rpc/Shelly.GetComponents',
            http_publish_funct: (value, self) => {
                let trv_ids = [];
                const valueObj = JSON.parse(value);
    
                for (const c of valueObj.components) {
                    if (c.key?.startsWith('blutrv:') && typeof c.config?.id === 'number') {
                        trv_ids.push({id: c.config.id, name: c.config?.name || ""});
                        self.adapter.log.warn("Found BLE device ... " + JSON.stringify(c.config));
                        self.adapter.log.warn("Found BLE device ... ID: " + c.config.id);                        
                    }
                }    
                return JSON.stringify(trv_ids);
            },
        },
        common: {
            name: { en: 'ID and name of paired BluTRV devices' },
            type: 'mixed',
            role: 'json',
            read: true,
            write: false,
...
/*
function addAvailableBluTRVs(deviceObj) {
    deviceObj[`PairedTRVs`] = {
        mqtt: {
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => JSON.stringify({
                id: self.getNextMsgId(),
                ...
                method: 'Shelly.GetComponents',
                params: { include: ['config'] },
                // TODO include config again and give name of TRV to BluTRV Init function
            }),
            ...
            http_publish_funct: (value, self) => {
                ...
                for (const c of valueObj.components) {
                    if (c.key?.startsWith('blutrv:') && typeof c.config?.id === 'number') {
                        self.adapter.log.warn("Adding BLE device ... " + JSON.stringify(c.config));
                        self.adapter.log.warn("Adding BLE device ... ID: " + c.config.id);
                        addBluTrvDevice(deviceObj, c.config.id);
                    }
                }    
            },
        }     
